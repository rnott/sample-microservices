 ...
 * <ul>
 * <li>${NULL} will be replaced with NUll value
 * <li>${SYSTIMESTAMP} will be replaced with current local date/time
 * <li>${SYSDATE} will be replaced with the current local date
 * <li>${SYSTIME} will be replaced with the current local time
 * </ul>
 */
public class TestDataSetLoader extends AbstractDataSetLoader {
	...
	protected IDataSet createDataSet( Resource resource ) throws Exception {
	    ...
	    Date now = new Date();
	    ds.addReplacementObject( "${SYSTIMESTAMP}", SYSTIMESTAMP.format( now ) );
	    ds.addReplacementObject( "${SYSDATE}", SYSDATE.format( now ) );
	    ds.addReplacementObject( "${SYSTIME}", SYSTIME.format( now ) );
